		...
		switch (error) {
			case 'utilisateur inconnu':
				...
				next = () => {
					...
					this.#password.classList.toggle('pwd-incorrect', true)
					this.id.select()
				}
				break
	...
	componentDidMount() {
		...
		if (this.fromEmailVerification !== undefined) {
			if (this.fromEmailVerification === '1')
				...
				this.context.addNotification({
					type: this.fromEmailVerification === '0' ? NotificationTypes.EMAIL_VERIFICATION_FAILED :
						NotificationTypes.EMAIL_VERIFICATION_SUCCEED
				})
		}
